...
export const CarouselPostCard = ({ post }: PostProps) => {
    return (
        <Link to={`/postagens/${post.id}`} className="w-full bg-zinc-800 rounded-xl hover:bg-zinc-100 shadow-md transition duration-700 hover:shadow-xl ">
...
export const PostSmCard = ({ post }: PostProps) => {
    return (
        <Link to={`/postagens/${post.id}`}>
            <li key={post.id} className="py-1 sm:py-2 hover:bg-zinc-100 transition duration-700 hover:shadow-lg rounded-md">
