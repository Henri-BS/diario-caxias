...
export const ProjectCard = ({ project }: ProjectProps) => {
    return (
        <Link to={`/projetos/${project.id}`} key={project.id} className="flex flex-col h-66 md:w-full md:h-40 items-start bg-zinc-100 border border-zinc-300 rounded-xl shadow-md md:flex-row  hover:shadow-xl transition duration-700 hover:scale-105">
            <img className="object-cover h-52 w-full md:h-40 md:w-40 rounded-t-xl  md:rounded-none md:rounded-s-xl" src={project.projectImage ? project.projectImage : "https://cdn1.iconfinder.com/data/icons/dashboard-ui-vol-1/48/JD-46-512.png"} alt={project.projectTitle} />
