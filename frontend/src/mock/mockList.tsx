...
export const CategoryMockList = () => {
    ...
    const filter = () => {
        return categoryMock.filter(item => item.categoryName.toUpperCase().includes(query.toLocaleUpperCase()));
    };
...
export const EventMockList = () => {
    ...
    const filter = () => {
        return eventMock.filter(item => item.eventTitle.toUpperCase().includes(query.toLocaleUpperCase()));
    };
...
export const PostMockList = () => {
    ...
    const filter = () => {
        return postMock.filter(item => item.postTitle.toUpperCase().includes(query.toLocaleUpperCase()));
    };
...
export const ProjectHomeMockList = () => {
    return (
        <>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-y-10 gap-x-6 items-start p-8 ">
...
export const CategoryMockHomeList = () => {
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-y-6 gap-x-4 items-start p-8 overflow-hidden">
                {categoryMock.map(category => {
                    categoryMock.length = 8
                    return (
                        <div key={category.id} className="overflow-hidden">
                            <Link href={`/categorias/${category.id}`} className="max-w-md text-center">
                                <div className="p-6 bg-zinc-100 border border-zinc-300 rounded-lg shadow-md transition duration-700 hover:shadow-xl hover:scale-105">
                                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 line-clamp-1">{category.categoryName}</h5>
                                </div>
                            </Link>
                        </div>
                    )
...
export const EventMockHomeList = () => {
    return (
        <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-6 items-start p-8">
                {eventMock.map(event => {
